'use strict';

/**
 * Created by vladthelittleone on 02.11.15.
 */
var documentation =
{
    world: {
        title: 'World',
        description: 'Объект, отвечающий за информацию о мире, его объектах (врагах, бонусах)',
        functions: [
            {
                name: 'getEnemies(callback)',
                description: [
                    'Функция возвращает массив всех врагов мира. Смотрите объект Enemy.',
                    'Параметр callback - функция с параметрами (e - вражеский космический корабль, i - индекс, arr - массив), которая вызывается для каждого элемента массива. callback - не обязательный параметр.'
                ]
            },
            {
                name: 'getBonuses(callback)',
                description: [
                    'Функция возвращает массив всех бонусов мира. Смотрите объект Bonus.',
                    'Параметр callback - функция с параметрами (e - вражеский космический корабль, i - индекс, arr - массив), которая вызывается для каждого элемента массива. callback - не обязательный параметр.'
                ]
            },
            {
                name: 'getBounds()',
                description: [
                    'Функция возвращает границы мира.',
                    'Пример возвращаемого объекта: {x: 0, y: 0, width: 2000, height: 2000}'
                ]
            }
        ]
    },
    spaceCraft: {
        title: 'SpaceCraft',
        description: 'Объект управления космическим кораблем',
        functions: [
            {
                name: 'weapon',
                description: [
                    'Вызов блока weapon для выполнения действий с оружием корабля.',
                    'weapon можно заменить на w.',
                    'Все методы, кроме работы с модулями, данного блока повторяются в самом spaceCraft объекте для удобства.',
                    'Смотрите объект Weapon.'
                ]
            },
            {
                name: 'engine',
                description: [
                    'Вызов блока engine для выполнения действий с двигателем корабля.',
                    'engine можно заменить на e.',
                    'Все методы, кроме работы с модулями, данного блока повторяются в самом spaceCraft объекте для удобства.',
                    'Смотрите объект Engine.'
                ]
            },
            {
                name: 'protection',
                description: [
                    'Вызов объекта protection для выполнения действий с защитными системами корабля.',
                    'protection можно заменить на p.',
                    'Все методы, кроме работы с модулями, данного блока повторяются в самом spaceCraft объекте для удобства.',
                    'Смотрите объект Protection.'
                ]
            },
            {
                name: 'getX()',
                description: [
                    'Функция возвращает координату корабля по оси х.'
                ]
            },
            {
                name: 'getY()',
                description: [
                    'Функция возвращает координату корабля по оси y.'
                ]
            },
            {
                name: 'getAngle()',
                description: [
                    'Получение текущего угла космического корабля.'
                ]
            },
            {
                name: 'angleBetween(another)',
                description: [
                    'Возвращает угол между космическим кораблем и объектом (another).'
                ]
            },
            {
                name: 'distance(another)',
                description: [
                    'Возвращает дистанцию от космического корабля до объекта (another).'
                ]
            },
            {
                name: 'getId()',
                description: [
                    'Функция вовращает id космического корабля.'
                ]
            },
            {
                name: 'getFreePoints()',
                description: [
                    'Получить информацию о количестве свободной энергии корабля.'
                ]
            },
            {
                name: 'getMaxEnergy()',
                description: [
                    'Получить информацию о количестве максимальной энергии корабля.'
                ]
            },
			{
				name: 'keyboardListener',
				description:[
					'Модуль обработки событий клавиатуры, необходим для реализации' +
					'ручного управления короблем.' +
					'Смотрите объект keyboardListener'
				]
			},
			{
				name: 'keyboard',
				description:[
					'Объект для задания параметра клавишы.'
				]
			}
        ]
    },
    engineBlock: {
        title: 'Engine Block',
        description: 'Блок, отвечающий за двигатели корабля. Храниться в SpaceCraft объекте: spaceCraft.engine.',
        functions: [
            {
                name: 'moveSpeed',
                description: [
                    'Модуль (см. Module объект) скорости передвижения корабля.',
                    'Предоставляет возможность изменять энергию данного модуля.'
                ]
            },
            {
                name: 'rotateLeft()',
                description: [
                    'Поворот влево космического корабля.'
                ]
            },
            {
                name: 'rotateRight()',
                description: [
                    'Поворот вправо корабля.'
                ]
            },
            {
                name: 'rotateTo(another)',
                description: [
                    'Поворот корабля к объекту (another).',
                    'Возвращает true, если нужен поворот, false, если нет (корабль ужен направлен на объект).'
                ]
            },
            {
                name: 'moveForward()',
                description: [
                    'Движение корабля вперед.'
                ]
            },
            {
                name: 'moveBackward()',
                description: [
                    'Движение корабля назад.'
                ]
            },
            {
                name: 'moveTo(x,y)',
                description: [
                    'Выполняется движение коробля к заданой точке или объекту.',
                    'Если задать x, y - как числовые значение, корабль полетит к заданным координатам.',
                    'Если в x передать объект бонуса или вражеского коробля, то ваш корабль полетит к нему',
                    '(Параметр y указывать не нужно)',
                    'В остальных случаях корабль просто будет двигатся вперед.'
                ]
            },
            {
                name: 'moveToNearestBonus()',
                description: [
                    'Выполняет перемещение коробля к ближайшему бонусу.'
                ]
            },
            {
                name: 'getMoveSpeed()',
                description: [
                    'Возврвщает текущую скорость корабля'
                ]
            }
        ]
    },
    weaponBlock: {
        title: 'Weapon Block',
        description: 'Блок управления оружием космического корабля. Храниться в SpaceCraft объекте: spaceCraft.weapon.',
        functions: [
            {
                name: 'rate',
                description: [
                    'Модуль (см. Module объект) скорости атаки оружия корабля.',
                    'Предоставляет возможность изменять энергию данного модуля.'
                ]
            },
            {
                name: 'range',
                description: [
                    'Модуль (см. Module объект) дальности поражения оружия корабля.',
                    'Предоставляет возможность изменять энергию данного модуля.'
                ]
            },
            {
                name: 'damage',
                description: [
                    'Модуль (см. Module объект) контроля урона, наносимого кораблем.',
                    'Предоставляет возможность изменять энергию данного модуля.'
                ]
            },
            {
                name: 'getDamage()',
                description: [
                    'Функция возвращает урон, наносимый оружием.'
                ]
            },
            {
                name: 'getFireRate()',
                description: [
                    'Функция возвращает скорострельность оружия.'
                ]
            },
            {
                name: 'getFireRange()',
                description: [
                    'Фуникция возвращает радиус поражения оружия.'
                ]
            },
            {
                name: 'inRange(another)',
                description: [
                    'Проверяет объект another, на вхождение в радиус поражения оружия.'
                ]
            },
            {
                name: 'fire(obj1, obj2)',
                description: [
                    'Выполняет выстрел в заданную точку.',
                    'obj1, obj2 могут быть координатой, тогда вызов будет: spaceCraft.fire(100, 100).',
                    'obj1 может быть объектом (врагом например), тогда вызов будет: spaceCraft.fire(enemy).',
                    'Вы так же можете не задавать параметры: spaceCraft.fire(). Выстрел будет по направлению корабля.'
                ]
            },
            {
                name: 'enemiesInRange(callback)',
                description: [
                    'Возвращает массив врагов в зоне поражения оружия.',
                    'Параметр callback - функция с параметрами (e - вражеский космический корабль, i - индекс, arr - массив), которая вызывается для каждого элемента массива. callback - не обязательный параметр.'
                ]
            },
            {
                name: 'fireNearestEnemy()',
                description: [
                    'Выполняет выстрел по ближайшему вражескому кораблю.'
                ]
            }
        ]
    },
    protectionBlock: {
        title: 'Protection Block',
        description: 'Блок управления защитными системами корабля. Храниться в SpaceCraft объекте: spaceCraft.protection.',
        functions: [
            {
                name: 'regeneration',
                description: [
                    'Модуль (см. Module объект) регенерации корабля.',
                    'Предоставляет возможность изменять энергию данного модуля.'
                ]
            },
            {
                name: 'getHealth()',
                description: [
                    'Функция возвращает уровень здоровья вашего корабля в данный момент.'
                ]
            },
            {
                name: 'getShield()',
                description: [
                    'Функция возвращает уровень щитов вашего корабля в данный момент.'
                ]
            },
            {
                name: 'getRegeneration()',
                description: [
                    'Функция возвращает текущую регенерацию корабля.'
                ]
            }
        ]
    },
    bonus: {
        title: 'Bonus',
        description: 'Объект обломков уничтоженного корабля (Бонус)',
        functions: [
            {
                name: 'getX()',
                description: [
                    'Функция возвращает координату обломков по оси х.'
                ]
            },
            {
                name: 'getY()',
                description: [
                    'Функция возвращает координату обломков по оси y.'
                ]
            },
            {
                name: 'getType()',
                description: [
                    'Функция возвращает тип бонуса. Всего их три:',
                    'Увеличение здоровья: health',
                    'Увеличение щита: shield',
                    'Увеличение урона оружия: damage',
                    'К примеру: bonus.getType() === "damage"'
                ]
            },
            {
                name: 'getId()',
                description: [
                    'Функция вовращает id обломков.'
                ]
            }
        ]
    },
    enemy: {
        title: 'Enemy',
        description: 'Объект, отвечающий за информацию о вражеском корабле.',
        functions: [
            {
                name: 'weapon',
                description: [
                    'Вызов объекта weapon для получение информации об оружии врага.',
                    'Смотрите объект EnemyWeapon.'
                ]
            },
            {
                name: 'getHealth()',
                description: [
                    'Функция возвращает уровень здоровья вражеского корабля в данный момент.'
                ]
            },
            {
                name: 'getShield()',
                description: [
                    'Функция возвращает уровень щитов вражеского корабля в данный момент.'
                ]
            },
            {
                name: 'getX()',
                description: [
                    'Функция возвращает координату корабля по оси х.'
                ]
            },
            {
                name: 'getY()',
                description: [
                    'Функция возвращает координату корабля по оси y.'
                ]
            },
            {
                name: 'getAngle()',
                description: [
                    'Получение текущего угла космического корабля.'
                ]
            },
            {
                name: 'angleBetween(another)',
                description: [
                    'Возвращает угол между космическим кораблем и объектом (another).'
                ]
            },
            {
                name: 'distance(another)',
                description: [
                    'Возвращает дистанцию от космического корабля до объекта (another).'
                ]
            },
            {
                name: 'getId()',
                description: [
                    'Функция вовращает id космического корабля.'
                ]
            }
        ]
    },
    enemyWeapon: {
        title: 'EnemyWeapon',
        description: 'Объект, отвечающий за информацию о оружии вражеского корабля',
        functions: [
            {
                name: 'getDamage()',
                description: [
                    'Функция возвращает урон, наносимый оружием.'
                ]
            },
            {
                name: 'getFireRate()',
                description: [
                    'Функция возвращает скорострельность оружия.'
                ]
            },
            {
                name: 'getFireRange()',
                description: [
                    'Фуникция возвращает радиус поражения оружия.'
                ]
            },
            {
                name: 'inRange(another)',
                description: [
                    'Проверяет объект another на вхождение в радиус поражения оружия.'
                ]
            },
            {
                name: 'enemiesInRange(callback)',
                description: [
                    'Возвращает массив врагов в зоне поражения оружия.',
                    'Параметр callback - функция с параметрами (e - вражеский космический корабль, i - индекс, arr - массив), которая вызывается для каждого элемента массива. callback - не обязательный параметр.'

                ]
            }
        ]
    },
    module: {
        title: 'Module',
        description: 'Объект, отвечающий за информацию о модуле блока.',
        functions: [
            {
                name: 'inc(i)',
                description: [
                    'Увеличивает энергию модуля корабля.',
                    'Без параметра увеличивает на 1 единицу энергии.',
                    'Для упрощения вы можете вызвать у spaceCraft объекта или у соответсвующего блока (w, e, p) методы:',
                    '- incRange(i)',
                    '- incRate(i)',
                    '- incDamage(i)',
                    '- incMoveSpeed(i)',
                    '- incRegen(i)',
                    'К примеру: spaceCraft.incRange(2) - увеличивает энергию модуля дальности атаки на 2.'
                ]
            },
            {
                name: 'dec(i)',
                description: [
                    'Уменьшает энергию модуля корабля.',
                    'Без параметра уменьшает на 1 единицу энергии.',
                    'Для упрощения вы можете вызвать у spaceCraft объекта или у соответсвующего блока (w, e, p) методы:',
                    '- decRange(i)',
                    '- decRate(i)',
                    '- decDamage(i)',
                    '- decMoveSpeed(i)',
                    '- decRegen(i)',
                    'К примеру: spaceCraft.decRange(2) - уменьшает энергию модуля дальности атаки на 2.'
                ]
            },
            {
                name: 'getMax()',
                description: [
                    'Возвращает максимальную энергию модуля.'
                ]
            },
            {
                name: 'getEnergyPoints()',
                description: [
                    'Возвращает количество потребляймой энергии модулем.',
                    'Для упрощения вы можете вызвать у spaceCraft объекта или у соответсвующего блока (w, e, p) методы:',
                    '- getRegenEnergy()',
                    '- getRateEnergy()',
                    '- getRangeEnergy()',
                    '- getMoveSpeedEnergy()',
                    '- getDamageEnergy()',
                    '- К примеру: spaceCraft.getRateEnergy() - возвращает количество потребляймой энергии модулем скорости атаки (Rate).'
                ]
            },
            {
                name: 'get(p)',
                description:[
                    'Функции подсчета значения от энергии модуля.',
                    'Для упрощения вы можете вызвать у spaceCraft объекта или у соответсвующего блока (w, e, p) методы:',
                    '- getRangeByPoints(p) - для дальности атаки',
                    '- getRateByPoints(p) - для скорости атаки',
                    '- getMoveSpeedByPoints(p)- для скорости корабля',
                    '- getDamageByPoints(p) - для показателя атаки',
                    '- getRegenByPoints(p) - для показателя регенерации'
                ]
            }
        ]
    },
	KeyboardListener:
	{
		title: 'Module',
		description: 'Объект отвечающий за обработку событий клавиатуры.',
		functions: [
			{
				name: 'addEventListener(key, callback)',
				description: [
					'Функция позволяет задать определенный набор действий завязанный на',
					'выбранную клавишу. Первый параметр клавиша, второй callback.',
					'Объект keyboard содержит константы определяющие код клавиши',
					'Стоит заметить, для того что бы активировать обработку событий клавиатуры',
					'необходимо вызвать метод init(), подробнее смотрите ниже.'
				]
			},
			{
				name: 'init()',
				description: [
					'Метод инициализирует все заданные события клавиатуры. Этот метод необходимо',
					'вызывать после всех вызывов addEventListener.',
					'Стоит заметить что вызовы функции addEventListener, после вызова этого метода',
					'будут проигнорированны.'
				]
			}
		]
	}
};
